...
def dynamic_distraction_decoder(config,
  ...
  with variable_scope.variable_scope(
    ...
    #initial_state = tf.constant(0, shape=(32, dim_1-1024),dtype=dtype)

    print (initial_state, encoder_sentence)

    init_s = tf.concat(1, [initial_state, encoder_sentence, query_sentence])
    ...
    if initial_state_attention:
      ...
      #attns_query = tf.constant(attns_query, shape=(attns_query))
      attns_query = [array_ops.concat(-1, (attns_query[0], query_sentence))]
      list_of_queries = [initial_state, attns_query[0],encoder_sentence]
      attns_state, _ = attention(list_of_queries)
      #attns_state = [tf.constant(0, shape(32,256),dtype=dtype)]
      attns_state = [array_ops.concat(-1, (attns_state[0], encoder_sentence))]
    prev_attn = None
    for i, inp in enumerate(decoder_inputs):
      ...
      if config.config_dir["is_distraction"] == True:
          ...
          if prev_attn is not None:
            temp =  math_ops.div(math_ops.reduce_sum(math_ops.mul(a, prev_attn),1),math_ops.reduce_sum(math_ops.mul(prev_attn,prev_attn),1) + eps)
            dummy = array_ops.transpose(prev_attn)

            t1 = math_ops.mul(dummy, temp)
            t1 = array_ops.transpose(t1)
            distract_output  = a  -  t1
            prev_attn += distract_output
          else:
            distract_output = a
      	  #distract_output, distract_state = distraction_cell(a, distract_state)
          #distract_output = tf.constant(0, shape=(32,256), dtype=dtype)
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        #attns_state = [tf.constant(0,shape=attns_state[0].get_shape(), dtype=dtype)]
        #attns_query = [tf.constant(0,shape=attns_query[0].get_shape(), dtype=dtype)]
  ...
  return outputs, state, attention_weights_para
...
def distraction_decoder_start(config,
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state, attention_weights_para = dynamic_distraction_decoder_wrapper(
            config,
            decoder_inputs,
            encoder_sentence,
            initial_state=encoder_state,
        ...
        return outputs + state_list + attention_weights_para
    ...
    attention_weights = outputs_and_state[-outputs_len:]
    state = state_list[0]
    if nest.is_sequence(encoder_state):
      state = nest.pack_sequence_as(structure=encoder_state,
                                    flat_sequence=state_list)
    return outputs_and_state[:outputs_len], state, attention_weights
