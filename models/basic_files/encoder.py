...
def encoder(config,
                    ...
                    initial_embedding_encoder = None,
                    initial_embedding_query = None,
                    num_heads=1,
                    ...
                    sequence_indices_encoder = None,
                    sequence_indices_query = None):
  ...
  """
  with variable_scope.variable_scope(scope or "dynamic_encoder", dtype=dtype) as scope:
    dtype = scope.dtype
    ...
    if initial_embedding_encoder is not None:
      embedding_sentence_encoder = variable_scope.get_variable('embedding_sentence_encoder',
        initializer=initial_embedding_encoder, trainable = False)
    else:
      print ("Error, Sentence Embedding missing \n")
      return

    if initial_embedding_query is not None:
      embedding_sentence_query = variable_scope.get_variable('embedding_sentence_query',
        initializer=initial_embedding_query, trainable=False)
    else:
      print ("Error, Sentence Embedding for Query is missing \n")
      return 


    if sequence_indices_query is not None and sequence_indices_encoder is not None:
      embedded_inputs_sentences = embedding_ops.embedding_lookup(embedding_sentence_encoder, sequence_indices_encoder)
      query_embeddings_sentences= embedding_ops.embedding_lookup(embedding_sentence_query, sequence_indices_query)
      embedded_inputs = array_ops.concat([embedded_inputs, embedded_inputs_sentences], 2)
      query_embeddings = array_ops.concat([query_embeddings, query_embeddings_sentences], 2)


    encoder_sentence = array_ops.unstack(embedded_inputs_sentences)[0]
    query_sentence = array_ops.unstack(query_embeddings_sentences)[0]
    ...
    top_states_encoder       = [array_ops.reshape(e, [-1, 1, encoder_size]) for e in encoder_outputs]
    ...
    return encoder_state,  attention_states_encoder, query_state, attention_states_query, scope, encoder_sentence, query_sentence
