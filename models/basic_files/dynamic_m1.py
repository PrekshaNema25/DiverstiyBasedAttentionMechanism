...
def dynamic_distraction_m1_decoder(decoder_inputs,
                      initial_state,
  ...
  with variable_scope.variable_scope(
      scope or "dynamic_distraction_m1_decoder", dtype=dtype) as scope:
    dtype = scope.dtype
    ...
    if initial_state_attention:
      ...
      attns_state, distract_prev_states = attention(list_of_queries, distract_prev_states)
...
def dynamic_distraction_m1_decoder_wrapper(decoder_inputs,
                                initial_state,
                                distract_initial_state,
                                attention_states,
                                attention_states_query,
  ...
  with variable_scope.variable_scope(
    embedding_scope or "dynamic_distraction_m1_decoder_wrapper", dtype=dtype,  reuse = True) as s1:
  ...
  with variable_scope.variable_scope(
    scope or "dynamic_distraction_m1_decoder_wrapper", dtype =dtype) as scope:
    return dynamic_distraction_m1_decoder(
        emb_inp,
        initial_state=initial_state,
        attention_states_query = attention_states_query,
        attention_states=attention_states,
        cell1 = cell_encoder, cell2 = cell_encoder,
...
def dynamic_distraction_m1_seq2seq(encoder_inputs,
                                decoder_inputs,
                                query_inputs,
                                cell_encoder_fw,
                                cell_encoder_bw,
  ...
  """
  with variable_scope.variable_scope(
      scope or "dynamic_distraction_m1_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
    # Encoder.
    ...
    with variable_scope.variable_scope("Encoder_Cell"):
      encoder_outputs, encoder_state_fw, encoder_state_bw = rnn.rnn(
          cell_encoder_fw,  embedded_inputs, dtype=dtype)

    with variable_scope.variable_scope("Query_Cell"):

      query_outputs, query_state_fw, query_state_bw = rnn.rnn(
        cell_encoder_fw, query_embeddings, dtype = dtype)
    ...
    query_state = query_state_fw
    top_states_encoder = [array_ops.reshape(e, [-1, 1, cell_encoder_fw.output_size])
                          for e in encoder_outputs]
    attention_states_encoder = array_ops.concat(1, top_states_encoder)

    top_states_query = [array_ops.reshape(e, [-1, 1, cell_encoder_fw.output_size]) for e in query_outputs]
    ...
    if isinstance(feed_previous, bool):
      return dynamic_distraction_m1_decoder_wrapper(
          decoder_inputs,
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state, ctx_vec = dynamic_distraction_m1_decoder_wrapper(
            decoder_inputs,
            initial_state=encoder_state,
        ...
        return outputs + state_list + ctx_vec
    ...
    ctx_vec = outputs_and_state[-outputs_len:]
    state = state_list[0]
    if nest.is_sequence(encoder_state):
      state = nest.pack_sequence_as(structure=encoder_state,
                                    flat_sequence=state_list)
    return outputs_and_state[:outputs_len], state, ctx_vec
